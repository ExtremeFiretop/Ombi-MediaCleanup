...
describe('LoginOAuthComponent', () => {
  ...
  it('should set pin from route params', () => {
    paramsSubject.next({ pin: 'opaque-token-12345' });
    expect(comp.pin).toBe('opaque-token-12345');
  });

  it('should save token and navigate on successful auth', () => {
    comp.pin = 'opaque-token-99999';
    comp.auth();
    expect(mockAuthService.oAuth).toHaveBeenCalledWith('opaque-token-99999');
